...
/* parse the arguments and ensure they are the correct type */
  ...
  if (get_cone_arr_dim("nuc_n", &(k->nuc_n), &(k->nucsize), cone) < 0) {
    free_py_scs_data(d, k, stgs, &ps);
  ...
  if (get_cone_arr_dim("sl_k", &(k->sl_k), &(k->sl_size), cone) < 0) {
    free_py_scs_data(d, k, stgs, &ps);
...
/* output arguments */
  ...
  info_dict = Py_BuildValue(
      ...
      "status", info.status,
      "ave_time_matrix_cone_proj", (scs_float)info.ave_time_matrix_cone_proj,
      "ave_time_vector_cone_proj", (scs_float)info.ave_time_vector_cone_proj);
      //"ave_time_eigendecomposition", (scs_float)info.ave_time_eigendecomposition,
      //"ave_time_recovery_matrix", (scs_float)info.ave_time_recovery_matrix);
  /* clang-format on */
