                              ...
                              Newton_stats *stats, bool *warm_start)
{
    ...
    //    to wrong point. We return if proj[0] >= t0 and the residuals suggest that
    //    we converged to the correct point.
    ...
    // ------------------------------------------------------------------------
    status = log_cone_Newton(t0, v0, x0, proj, n, workspace, stats,
                             warm_start);
                                    ...
                                    scs_float residuals[3], scs_float *dualx)
{
    ...
    // ---------------------------------------------------------------------
    complementarity = tvx[0] * dualt + tvx[1] * dualv +
                      SCS(dot)(dualx, tvx + 2, n);
