...
static void KKT_precompute(scs_float v, const scs_float *x_inv,
                           const scs_float *z, const scs_float *w,
                           scs_int n, KKT_solver_workspace *KKT_work)
{
    scs_int u_dim = n + 3;
    ...
    GinvC[n + 4] = (g0g1[n + 4] + z[1] * SCS(dot)(g0g1 + n + 5, temp2, n)) /
                   coeff;
    for (scs_int i = n + 5; i < 2 * n + 5; i++)
    {
        GinvC[i] = (g0g1[i] + (z[1] * GinvC[n + 4]) * x_inv[i - n - 5]) /
                   temp1[i - n - 5];
    }
...
static void KKT_solve(const scs_float *z, const scs_float *w,
                      ...
                      scs_float *du1, scs_float *dr, scs_float *dz, scs_float *ds)
{
    scs_int u_dim = n + 3;
    scs_int u1_dim = n + 2;
    ...
    *dr = 0;
    ...
    for (size_t i = 0; i < num_ref; ++i)
    {
        ...
        RinvCTGinvRes[2] = (CTGinvRes[0] - R[0] * RinvCTGinvRes[0] -
                            R[3] * RinvCTGinvRes[1]) /
                           R[6];
        ...
        *dr += GinvRes[n + 2];
        ...
        if (i != num_ref - 1)
        {
            ...
            scs_float coeff1 = SCS(dot)(g0g1 + u_dim, du1, u1_dim) +
                               g0g1[2 * n + 5] * (*dr);
                     ...
                     scs_int variant)
{
    ...
    for (iter = 0; iter < MAX_ITER_IPM; ++iter)
    {
        ...
        scs_float pres = sqrt(rznl[0] * rznl[0] + rznl[1] * rznl[1] +
                              rznl[2] * rznl[2]);
        ...
        for (size_t i = 0; i < 2; ++i)
        {
            ...
            KKT_solve(z, w, x_inv, u1[1], lmbda, n, rhs1_aff, rhs2_aff,
                      &KKT_work, du1, &dr, dz, ds);
        ...
        // --------------------------------------------------------------------
        // check if we still make sufficient progress
        // --------------------------------------------------------------------
